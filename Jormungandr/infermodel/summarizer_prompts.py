...
def build_window_prompt(
    ...
    *,
    ...
    return (
        ...
        "- 不要输出思考过程、推理过程、reasoning 或 thinking。\n"
...
def build_fusion_prompt(
    ...
    *,
    ...
    return (
        ...
        "- 除 `summary` 和 `detailed_summary` 外，还要输出 plot arc 级结构字段。\n"
        "- 所有结构字段只能基于当前 plot 覆盖章节判断；不要判断全书地位、全书长期后果、全书主线结论。\n"
        "- `plot_function` 从这些中文标签中选择 1 到 6 个：开篇钩子、人物出场、世界观展示、冲突铺垫、冲突引爆、误会制造、关系升温、关系降温、关系决裂、能力觉醒、资源获取、任务发布、副本进入、危机升级、秘密揭露、伏笔埋设、伏笔回收、爽点释放、打脸反杀、刀点爆发、阶段胜利、阶段失败、过渡、终局收束。\n"
        "- `driving_force` 从这些中文标签中选择 1 到 5 个：升级驱动、复仇驱动、生存驱动、调查驱动、恋爱驱动、关系修复驱动、任务驱动、副本驱动、经营驱动、战争驱动、权谋驱动、逃亡驱动、成长驱动、身份揭露驱动、秘密揭开驱动、资源争夺驱动。\n"
        "- `key_events` 提取 2 到 8 个关键事件，按情节发生顺序排列。`chapter_order` 填最相关章节序号，无法唯一定位时填该事件开始章节。\n"
        "- `characters_involved` 只列对该 plot 推进有作用的人物，不要列背景路人。\n"
        "- `relationship_changes` 只列当前 plot 中确实发生变化的人物关系；没有则输出空数组。\n"
        "- `conflict_model` 要把表层冲突和深层冲突分开，`stakes` 写当前 plot 内的风险或代价。\n"
        "- `payoff_and_hook` 记录当前 plot 内的爽点、虐点/压迫点、悬念钩子和结尾钩子；没有依据的项目用空数组或空字符串。\n"
        "- `setup_and_resolution` 记录当前 plot 开始时提出的问题、结束时解决的问题、尚未解决的问题和新制造的问题。\n"
        "- `abstraction_hint` 只给后续抽象模型的线索，不要做全书级定论。\n"
