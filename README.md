...
- 校验后再提升到 `Library/rawdata/novels/` 或 `Library/rawdata/stories/`
...
- `Library/rawdata/novels/book_xxxx/`
- `Library/rawdata/stories/story_xxxx/`
...
- `Library/rawdata/novels/...`

输出：

- `Library/reference/facts/cleaned_chapters/book_xxxx/`
...
- 读取 `cleaned_chapters` 的 `index.json + chapter_xxxx.json`
...
- `Library/reference/facts/chapter_features/book_xxxx/`
- `.softmodel.done`
...
- `Library/reference/facts/plot_segments/book_xxxx/`
- `.infermodel.done`
...
- `Library/reference/facts/plot_segments/book_xxxx/`
- 可选 `Library/reference/facts/chapter_features/`
...
- `rawdata/`：抓回来的原始文本
- `reference/facts/`：结构化事实层
- `reference/abstractions/`：抽象经验层，适合放套路、人物弧、节奏模型
- `ideas/`：灵感和种子素材
- `indexes/`：注册表、索引、优先队列
...
1. 外部网页内容先进入 `rawdata`
2. `rawdata` 被 hardmodel 变成规范章节事实 `cleaned_chapters`
3. `cleaned_chapters` 被 softmodel 变成章节语义事实 `chapter_features`
4. `chapter_features` 被 infermodel 变成全书剧情结构 `plot_segments`
5. `plot_segments` 被 generatemodel 重新组合，生成新的书籍草案

也就是说，项目的主干数据对象依次是：

`原始文本 -> 清洗章节 -> 章节语义特征 -> 情节片段 -> 生成草案`
...
- `Library/reference/facts` 和 `Library/reference/abstractions` 的拆分方向是对的，便于从“事实”上升到“创作规律”
...
`Library/reference/abstractions/` 已经预留了大量目录，但当前主链路主要停在 facts 层。也就是说，项目已经能抽取“发生了什么”，但还没有稳定沉淀“为什么这个结构有效”。
...
### 建议 3：把 `facts -> abstractions` 做成正式阶段

这是项目真正能拉开差距的地方。建议新增一个明确阶段，例如：

`chapter_features / plot_segments -> abstractions`
...
```bash
fetcher-run <url>
hardmodel-run Library/rawdata/novels --use-clean-registry
softmodel-run Library/reference/facts/cleaned_chapters
infermodel-run Library/reference/facts/chapter_features --api-key "$MIMO_API_KEY"
generatemodel-run Library/reference/facts/plot_segments
```
...
一个以 `Library` 为知识底座、以 `facts` 为结构中间层、以 `Projects/_generated` 为生成出口的小说语料与创作流水线。
