...
class PlotSegmentMerger:
    ...
    def merge(
        ...
        *,
        ...
        plots: list[GlobalPlot] = []
        current_orders: list[int] = []
        for order in chapter_orders:
            current_orders.append(order)
            if order in selected_boundaries:
                plots.append(self._build_plot(len(plots) + 1, current_orders, order_to_chapter, boundary_stats, window_results, analyzer))
                current_orders = []
        if current_orders:
            plots.append(self._build_plot(len(plots) + 1, current_orders, order_to_chapter, boundary_stats, window_results, analyzer))
    ...
    def _select_boundaries(
        ...
        *,
        ...
        for boundary, stats in boundary_stats.items():
            validation = None
            if analyzer is not None and boundary in order_to_index and boundary + 1 in order_to_index:
                validation = self._validate_boundary(boundary, ordered_chapters, order_to_index, analyzer)
    ...
    def _build_plot(
        ...
        if analyzer is not None:
            summary, detailed_summary = analyzer.fuse_plot_summaries(
                plot,
                chapters,
